...
Följande stycke beräknar kvoten av kolumnerna valence och danceability i en ny kolumn `val_to_dan`.

```{r, eval = F}
dat %>%                                                    # Ta datan, och sen
  mutate(val_to_dan = valence / danceability) %>%          # beräkna valence delat på danceability, och sen
  select(valence, danceability, val_to_dan)                # välj relevanta kolumner.
```
...
```{r, eval = F}
ggplot(dat_small, aes(x = mode_name, y = valence, color = album_name)) + # Ta datan och koppla variabler till egenskaper
  geom_point() +                                                         # Illustrera med punkter
...
Vad ska ändras i stycket ovan för att skapa en graf över medelvalens (`valence`) istället för dansbarhet? Är barnvänlig musik gladare eller ledsnare är icke-barnvänlig?
:::
