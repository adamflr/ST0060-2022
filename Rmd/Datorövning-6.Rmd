...
---
# Datorövning 6. Test för två stickprov
...
Konfidensintervallet beräknas från differenserna på samma sätt som vid ett stickprov med normalfördelad data. Tolkningen liknar den före ett stickprov: med 95 procents konfidens ligger den sanna skillnaden i medelvärden i intervallet.
...
# dat_onion <- read_csv("https://raw.githubusercontent.com/adamflr/ST0060-2022/main/Data/Uppgiftsdata/Uppgift_L%C3%B6kf%C3%A4rg.csv") # Alternativ lösning
```
:::
...
```{r, eval = F}
dat_long <- dat_onion %>% pivot_longer(-Odlare)

ggplot(dat_long, aes(name, value, group = Odlare)) +
  geom_point() +
  geom_line()
```
...
# dat_sq <- read_csv("https://raw.githubusercontent.com/adamflr/ST0060-2022/main/Data/Uppgiftsdata/Uppgift_Ekorrar.csv") # Alternativ lösning
```
:::
...
# dat_pot <- read_csv("https://raw.githubusercontent.com/adamflr/ST0060-2022/main/Data/Uppgiftsdata/Uppgift_Po-ta-toes.csv")
```
:::

::: {.hypothesis name="Po-ta-toes-graf"}
...
::: {.hypothesis name="Po-ta-toes-test"}
...
::: {.exercise name="Chi-två-test för hemmasegrar"}
Använd ett chi-två-test för att se om proportionen hemmasegrar förändrats under hela tidsperioden.
```{r, eval = F}
dat_hemma[, c(2,3)]
chisq.test(___)
```
...
::: {.exercise name="Är det ens en rimlig sak att testa"}
Vid ett hypotestest har vi en population och ett stickprov. Är det rimligt att se alla matcher i allsvenskan som ett stickprov? Vad är i så fall populationen? Till vilka andra situationer kan resultat från den här datan generaliseras?
:::
