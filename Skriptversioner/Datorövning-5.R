...
# `count()` med ett logiskt villkor som beräkningsvariabel. De rader där `bortamal > hemmamal` är
# bortasegrar.
#
...
#
# I R kommer man förstås inte beräkna ett z-test genom att gå igenom samtliga steg, då det finns en
# egen funktion för test av proportioner `prop.test()`. Funktionens argument ges av *antalet*
# positiva utfall och det totala antalet observationer. Nollhypotesens värde anges med ett argument
# `p`. Det finns också en möjlighet att förbättra testet med en så kallad kontinuitetskorrektion. Då
# det inte ingår i kursen måste vi sätta `correct = F` för att få samma utfall som handräkningen. Vi
# kan genomföra testet med
#
...
# inte z-värdet utan ett chi-två-värde (2.5661). Här är det värdet z-värdet i kvadrat.
#
...
#
# För att få de förväntade värdema E multipliceras nollhypotesens sannolikheter med det totala
# antalet matcher.
#
