...
#
# Inom vetenskapen är Excel, på gott och ont, det vanligaste filformatet för mindre datamängder.
...
# `aes()`-funktionen och sätter argumentet `data`. Det kan också vara informativt att använda
# `geom_text()` för länderna. Etiketterna sätts med argumentet `label` i `aes()`. (Om man oroar sig
# för överlappande etiketter och letar efter lite överkurs kan man titta på paketet `ggrepel`.)
#
...
#
# Det vara nyttigt att göra en stegvis beräkning av varians (och standardavvikelse) för att få en
# lite bättre förståelse av vad det faktiskt beskriver. Nedan ges en slumpmässig exempeldata med fem
# observationer. Om du har en tärning från datorövning 1 kan du med fördel kasta den nu, och ersätta
# värdena med dina egna utfall
#
...
#
# Vi har tidigare sett beräkningen av medelvärden med `mean()`. En skillnad mot tidigare är att vi
# gör det i ett `mutate()` steg istället för `summarise()`, eftersom vi vill ha kvar den tidigare
# strukturen med flera rader.
#
...
#
# Därefter tar vi kvadrater av differenserna. För att ange en kvadrat används symbolen `^2`, t.ex.
# `3^2` för tre i kvadrat.
#
...
#
# Som avslutande steg delar vi kvadratsumman med antalet observationer minus ett. För fem
# observationer delar vi alltså med fyra.
#
...
#
# Förra gången anvnäde vi paketet `plotly` för att göra en interaktiv graf. Paketet har också
...
# finns en tabell med dataset publicerade under 2022. Gå igenom listan, hitta ett intressant fall
# och läs informationen om det (genom att klicka på länken). Är det möjligt att förstå beskrivningen
# av datan?
# :::
#
